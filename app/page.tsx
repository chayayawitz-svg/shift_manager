...
export default function Home() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // 2. שליחה למייל (שולחים רק את המספרים!)
    ...
    <div className="min-h-screen p-4 bg-[#020414] text-right font-sans" dir="rtl">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-start">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[40px] p-8 lg:p-12 flex-1 shadow-2xl w-full border-t-8 border-[#FF3366]">
          ...
          <div className="space-y-8 max-h-[500px] overflow-y-auto pl-2 custom-scrollbar">
            {sections.map(s => (
          ...
          <button disabled={isSubmitting} className="w-full mt-10 bg-[#FF3366] text-white py-5 rounded-full font-bold text-xl shadow-lg active:scale-95 transition">
        ...
        {/* תצוגה מקדימה למפה (Recharts ליופי בלבד במסך) */}
        <div className="flex-1 bg-[#020414] rounded-[40px] p-8 flex flex-col items-center justify-center min-h-[600px] border border-white/10">
          <div className="bg-[#FF3366] px-8 py-2 rounded-full mb-8 text-white font-bold text-xl italic">
