...
// רשימת 6 מיומנויות הליבה לפי התמונה החדשה
const sections = [
...
/* -------------------------------------------------
   רכיבי עיצוב ושחזור המראה המקורי
------------------------------------------------- */
...
export default function Home() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // שמירה ל-Supabase - התאמה ל-6 עמודות
    ...
    <div className="min-h-screen p-4 bg-[#020414] text-right" dir="rtl">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-start">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-[40px] p-8 lg:p-12 flex-1 shadow-2xl w-full">
          ...
          {status === "error" && <p className="text-red-600 text-center mt-6 font-bold">הייתה שגיאה בשליחה. נסי שנית.</p>}
        </form>
