  ...
  return (
    <div className="min-h-screen p-8 bg-[#f4f7f6] text-right" dir="rtl">
      <div className="max-w-6xl mx-auto">
        ...
        {isLoading ? (
          <p className="text-center mt-10 font-bold">טוען נתונים מהמסד...</p>
        ) : (
          <>
            {/* כרטיסי סיכום */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
              <div className="bg-white p-6 rounded-2xl shadow-md border-r-4 border-blue-900">
                <p className="text-gray-500 font-bold text-sm">סה"כ משיבים</p>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-md border-r-4 border-[#FF3366]">
                <p className="text-gray-500 font-bold text-sm">ממוצע הובלה צוותי</p>
            ...
            {/* גרף עוגה משופר */}
            <div className="bg-white p-8 rounded-2xl shadow-xl mb-10 overflow-hidden">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart margin={{ top: 40, right: 80, bottom: 40, left: 80 }}>
                    <Pie
                      ...
                      labelLine={{ stroke: '#4b5563', strokeWidth: 1.5 }}
                      outerRadius={100}
                      innerRadius={65}
                      paddingAngle={8}
                      dataKey="value"
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
               <div className="bg-[#0b1a40] text-white p-4 font-bold text-xl text-center">פירוט תשובות מלא</div>
               <div className="overflow-x-auto">
                <table className="w-full text-right border-collapse">
                  <thead>
                    <tr className="bg-gray-50 text-blue-900 border-b border-gray-200 uppercase text-xs font-bold">
                      <th className="p-4">תאריך</th>
                      <th className="p-4">שם מלא</th>
                      <th className="p-4">הובלה</th>
                      <th className="p-4">נשמה</th>
                  ...
                  <tbody>
                    {results.map((row) => (
                      <tr key={row.id} className="border-b border-gray-50 hover:bg-blue-50/30 transition-colors">
                        <td className="p-4 text-sm text-gray-500 font-mono italic">
                          {new Date(row.created_at).toLocaleDateString('he-IL')}
                        </td>
                        <td className="p-4 font-bold text-blue-900">{row.full_name}</td>
