...
export default function DashboardPage() {
  ...
  // פונקציית כיתובים מתוקנת - שבירת שורות ומרחק אחיד
  ...
  return (
    <div className="min-h-screen p-6 bg-[#f8fafc] text-right font-sans" dir="rtl">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl font-black text-blue-900 mb-10 text-center uppercase tracking-tight">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex justify-center">
              <div className="bg-white p-5 rounded-3xl shadow-sm border-b-4 border-blue-900 text-center min-w-[280px]">
            ...
            {/* תיבת הגרף - מותאמת אישית */}
            <div className="bg-white p-6 lg:p-8 rounded-[40px] shadow-sm border border-gray-100 max-w-5xl mx-auto w-full">
              <h2 className="text-2xl font-black mb-4 text-blue-900 text-center">התפלגות חוזקות</h2>
            ...
            {/* טבלה ומסננים */}
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden mt-4">
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 bg-white/5 p-4 rounded-3xl border border-white/10">
                    ...
                    <div className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-xl px-3 py-1">
                      ...
                      <input type="date" className="bg-transparent text-white text-xs outline-none invert brightness-200" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                      <span className="text-white/20 text-xs px-1">עד</span>
                      <input type="date" className="bg-transparent text-white text-xs outline-none invert brightness-200" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                    </div>
                    <div className="relative">
                    ...
                    <div className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-xl px-3">
                      ...
                      <input type="number" min="0" max="10" placeholder="0" className="bg-transparent text-white text-sm w-full py-2 outline-none" value={minScore} onChange={(e) => setMinScore(e.target.value)} />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full text-right border-collapse">
                  ...
                  <tbody>
                    {filteredResults.length > 0 ? filteredResults.map((row) => {
                      // לוגיקה לזיהוי הגשות חוזרות (חיה 1, חיה 2...)
                      ...
                      return (
                        <tr key={row.id} className="border-b hover:bg-blue-50/30 transition-colors group">
                          <td className="p-5 text-xs text-gray-400 font-medium">{new Date(row.created_at).toLocaleDateString('he-IL')}</td>
                          <td className="p-5 font-bold text-blue-900">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex flex-col text-right">
                                <span className="text-base leading-tight">{row.full_name}</span>
                                {isMultiple && (
                      ...
                      <tr><td colSpan={8} className="p-24 text-center opacity-30 text-xl font-bold">לא נמצאו תוצאות</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
